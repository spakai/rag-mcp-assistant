...
- [x] Uploading a `.txt` or `.pdf` object under `documents/` to the bucket
      triggers the ingestion function.
- [x] The document's text is extracted: UTF-8 text for `.txt`; the embedded text
      layer for `.pdf`.
- [x] The text is split into chunks of a configurable size (default ~1000
...
- [x] Each chunk is stored as its own record with: `document_id`, `chunk_index`,
      `text`, `char_count`, `source_key`, and `created_at`.
- [x] A document-level record is stored with: `document_id`, `source_key`,
      `chunk_count`, `status = "ingested"`, and `created_at`.
- [x] Re-uploading the same object key replaces that document's chunks rather
      than duplicating them (ingestion is idempotent per source key).
- [x] Unit tests cover the chunking logic: chunk sizes, overlap, and the final
      short chunk, using mocked AWS clients (no network).
- [x] An integration test confirms the chunk records and the document record
